...
export function WorkflowPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">نظام Workflow</h1><p className="text-sm text-muted-foreground">M14: تعريف المسارات ومتابعة التنفيذ.</p></div>
      <section className="rounded-xl border bg-background p-4 flex flex-wrap items-center gap-2">
        <input value={workflowName} onChange={(e)=>setWorkflowName(e.target.value)} placeholder="اسم Workflow الجديد" className="h-9 rounded-md border px-3 text-sm min-w-64" />
        <button onClick={createWorkflow} className="h-9 px-3 rounded-md border text-sm hover:bg-muted">Add workflow</button>
      </section>
      <section className="rounded-xl border bg-background p-4">
        <h2 className="font-semibold mb-3">تعريفات Workflow</h2>
