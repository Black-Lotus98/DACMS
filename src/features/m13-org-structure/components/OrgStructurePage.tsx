    ...
    <div className="space-y-6">
      ...
      <section className="rounded-xl border bg-background p-4 grid lg:grid-cols-3 gap-4">
        <div className="space-y-2">
          <h2 className="font-semibold text-sm">إضافة فرع</h2>
          <input value={branchName} onChange={(e)=>setBranchName(e.target.value)} placeholder="اسم الفرع" className="w-full h-9 rounded-md border px-3 text-sm" />
          <input value={branchCode} onChange={(e)=>setBranchCode(e.target.value)} placeholder="رمز الفرع" className="w-full h-9 rounded-md border px-3 text-sm font-mono" />
          <button onClick={createBranch} className="h-9 px-3 rounded-md border text-sm hover:bg-muted">إضافة</button>
        </div>
        <div className="space-y-2">
          <h2 className="font-semibold text-sm">إضافة إدارة</h2>
          ...
          <input value={deptName} onChange={(e)=>setDeptName(e.target.value)} placeholder="اسم الإدارة" className="w-full h-9 rounded-md border px-3 text-sm" />
          <input value={deptCode} onChange={(e)=>setDeptCode(e.target.value)} placeholder="رمز الإدارة" className="w-full h-9 rounded-md border px-3 text-sm font-mono" />
          <button onClick={createDepartment} className="h-9 px-3 rounded-md border text-sm hover:bg-muted">إضافة</button>
        </div>
        <div className="space-y-2">
          <h2 className="font-semibold text-sm">تعيين مسؤول الإدارة</h2>
          ...
          <button onClick={assignResponsible} className="h-9 px-3 rounded-md border text-sm hover:bg-muted">حفظ</button>
        </div>
      </section>
