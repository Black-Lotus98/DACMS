...
export function SearchPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">البحث والاسترجاع</h1><p className="text-sm text-muted-foreground">M4: بحث متقدم مع استعلامات محفوظة.</p></div>
      <div className="grid sm:grid-cols-3 gap-2">
        <input value={q} onChange={(e)=>setQ(e.target.value)} placeholder="ابحث بالعنوان أو المرجع..." className="w-full h-10 rounded-md border px-3 sm:col-span-2" />
        <select value={secrecy} onChange={(e)=>setSecrecy(e.target.value)} className="w-full h-10 rounded-md border px-3">
          <option value="">كل مستويات السرية</option>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button onClick={saveCurrentQuery} className="h-9 px-3 rounded-md bg-primary text-primary-foreground text-sm">حفظ الاستعلام</button>
      </div>
